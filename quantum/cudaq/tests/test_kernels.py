...
from cudaq.kernels import (  # noqa: E402
...
from qiskit.kernels import (  # noqa: E402
...
def test_train_fraud_model() -> None:
    ...
    # The model should at least update weights away from zero
...
def test_variational_fraud_classifier_extremes() -> None:
    # Very positive features/weights → score ≈ 1
    s_hi = variational_fraud_classifier([100, 100, 100], [100, 100, 100])
    # Very negative features/weights → score ≈ 0
    s_lo = variational_fraud_classifier([-100, -100, -100], [100, 100, 100])
    assert s_hi > 0.99
    assert s_lo < 0.01
